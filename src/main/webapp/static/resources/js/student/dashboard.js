function dashboardStudentController($scope, model, sharedProperties, $window, $sce, $location) {
    $scope.sharedProperties = sharedProperties
    $scope.user = model.user
    $scope.courses = model.courses
    $scope.comments = model.comments
    $scope.commentsLimit = 7
    if ($location.search() && $location.search().commentsLimit) {
        $scope.commentsLimit = $location.search().commentsLimit
    }

    $scope.showMoreMsg = function() {
        return !($location.search() && $location.search().commentsLimit)
    }
...
dashboardStudentController.resolve = {
    model : function($q, $http) {
        var deferred = $q.defer();

        $http({method: 'GET', url: '/student/dashboard'})
