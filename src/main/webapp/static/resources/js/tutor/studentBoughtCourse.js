function studentBoughtCourseController($scope, model, sharedProperties, $http, $location, audio, $modal, courseUtils, $state) {
    ...
    $scope.courseGroupComments = []
    $scope.lessonsComments = []
    /*******************
     * Страницы, переходы
     *******************/
    $scope.pages = []
...
//            {num: 4, id: 'files', type: 'tab', setIt: function() {$scope.setTab('files')}}
    ...
    $scope.download = function (item) {
        var $form = $("<form action='" + "/fileBox/download/" + item.entryFile.id + "'></form>")
        $form.appendTo("body")
        $form.submit()
    }
    /*******************
    ...
    $scope.play = function (item) {
        if (item.entryFile.entryFileType.type == 'audio') {
            var url = "/fileBox/download/" + item.entryFile.id;
...
studentBoughtCourseController.resolve = {
    model: function ($q, $http, $stateParams) {
        var deferred = $q.defer();

        $http({method: 'GET', url: '/tutor/coursePreview/' + $stateParams.courseId})
            .success(function (data) {
